# Integrated Cave Flood Simulation — thesis-enhanced version
# --------------------------------------------------------------------
...
#    Blumberg and Curl (1974) implicit equation, following Springer (2004)
...
#-------------------------------------------------------------
# Important framing for the thesis:
# - This is a 1D steady head-loss model, not a full transient CFD solver.
# - The flood pulse is implemented as repeated steady snapshots for scenario analysis.


# re is used to clean and parse survey text lines.
# json is used to save a metadata summary of the model run.
import re
import json
...
# Goal: pull out only the usable survey shots and ignore headers/noise.
# Each valid line should begin with an MFS station name and contain:
# from-station, to-station, shot length, azimuth, inclination, left, up, down, right.
# The survey file is stored in feet, so every distance value is converted to meters here.
# Output = a list of cleaned survey shots that the rest of the model can use.
def parse_survey_file(filename: str):
    # Read the Compass-style survey text file and keep only the station-to-station
    ...
    with open(filename, "r", encoding="utf-8", errors="ignore") as f:
        for raw in f:
            ...
            parts = line.split(" ")
            # Require at least 9 fields: from, to, length, azimuth, inclination, L, U, D, R.
            if len(parts) < 9:
                continue
            try:
                frm, to = parts[0], parts[1]

                # survey.txt values are in feet, so convert everything to meters here.
...
# Think of it as "walking" through the cave one shot at a time.
# For each shot:
#   - use azimuth + inclination to break the length into x, y, z parts
#   - add that step onto the previous station location
# This produces one XYZ point per station so the cave passage has a mappable path in space.
def survey_to_xyz(shots):
    ...
    pts = []
    x = y = z = 0.0
    # Start the centerline at an arbitrary origin (0,0,0).
    pts.append((x, y, z))
...
# LRUD means left, right, up, down distances measured at each survey station.
# Here those are treated as an ellipse:
#   width  = left + right
#   height = up + down
#   area   = pi*a*b
# Then the ellipse perimeter is approximated with the Ramanujan formula.
# Hydraulic radius is then computed as A/P.
# Output = cross-sectional area and hydraulic radius at each node.
def lrud_geometry_from_shots(shots) -> Tuple[np.ndarray, np.ndarray]:
    """
    Convert LRUD survey shots to elliptical cross-section geometry.
    Area = pi*a*b, Rh = A/P following Springer (2004) Fig. 1.
    Returns A_lr, Rh_lr (drops P_ram as a separate return — Rh already encodes it).
    """
    ...
    # Approximate the wetted perimeter using Ramanujan's ellipse formula.
    Aax = np.maximum(a, b)
    Bax = np.minimum(a, b)
...
# In your thesis model, this is mainly used as a diagnostic and to support
# roughness / friction calculations.
def reynolds(V: np.ndarray, De: np.ndarray, nu: float) -> np.ndarray:
    # Reynolds number describes whether flow is more laminar or turbulent.
...
def hf_darcy(ds: np.ndarray, V: np.ndarray, De: np.ndarray, f: np.ndarray) -> np.ndarray:
    # Darcy-Weisbach head loss along each segment.
...
# First compute the friction slope Sf from n, velocity, and hydraulic radius.
...
def hf_manning(ds: np.ndarray, V: np.ndarray, Rh: np.ndarray, n: float) -> np.ndarray:
    # Manning-based head loss along each segment.
    ds = np.asarray(ds, float)
    V = np.asarray(V, float)
...
# This method uses discharge and diameter instead of friction factor.
# It is traditionally used for pressurized pipe flow, so here it is being used
# as a comparison method against Darcy and Manning under the same geometry.
def hf_hazen(ds: np.ndarray, Q: np.ndarray, De: np.ndarray, C: float) -> np.ndarray:
    # Hazen-Williams head loss along each segment.
    ds = np.asarray(ds, float)
...
# Purpose: estimate Darcy friction factor from Reynolds number and roughness.
# If Re is low, the code uses the laminar solution f = 64/Re.
# If Re is turbulent, it uses the Swamee-Jain explicit approximation.
# Here it mainly provides a starting guess for later friction calculations.
def swamee_jain_array(Re: np.ndarray, De: np.ndarray, k) -> np.ndarray:
    # Estimate Darcy friction factor from Reynolds number and roughness.
    ...
    # Otherwise use the explicit Swamee-Jain turbulent approximation.
    tur = ~lam
...
# Following Springer (2004) Eq. 4
# --------------------------------------------------------------------
# This is the scallop-based friction calibration block.
# Feed it measured/derived anchor velocities and hydraulic diameters.
# For each anchor site, it solves the Blumberg & Curl implicit equation
...
# Need containers here to hold Reynolds number and friction factor for all sites.
# Final output = one friction factor per anchor site + one reach-average value
...
def blumberg_curl_ff(
    ...
    """
    Estimate the Darcy friction factor at each scallop anchor station
    using the implicit equation of Blumberg and Curl (1974):

        ff^(-1/2) = 1.77 * ln(Re_m * ff^(1/2)) - 11.44

    where Re_m is the Reynolds number from scallop-derived velocity
    and hydraulic diameter at each site.

    The reach-averaged ff (mean across all anchor stations) is applied
    as a single uniform friction factor throughout the passage,
    following Springer (2004).

    Parameters
    ----------
    V_anchor  : scallop-derived velocities at each anchor site (m/s)
    De_anchor : hydraulic diameters at each anchor site (m)
    fluid     : FluidProps (provides kinematic viscosity nu)
    n_iter    : maximum iterations for implicit solve

    Returns
    -------
    ff_reach : single reach-averaged friction factor (float)
    diag_df  : DataFrame with per-site diagnostics
    """
    ...
    print(f"  Reach-averaged ff = {ff_reach:.4f}  "
          f"(Blumberg & Curl 1974; Springer 2004)")
...
# Feed it the cave geometry, one discharge Q, the roughness coefficients,
# and the method you want to use (darcy, manning, or hazen).
# At this stage the model treats the flood as one steady "snapshot" rather than
# a fully unsteady transient event.
# Steps inside this block:
#   1) convert discharge into node velocities using V = Q/A
#   2) average node values onto passage segments
#   3) compute head loss by the chosen hydraulic equation
#   4) add minor losses from expansions/contractions
#   5) accumulate losses downstream to build HGL and EGL
# Output = one complete hydraulic profile along the passage.
def simulate_snapshot(
    ...
    # Average node values to segment values because head loss is computed per segment.
    ...
    # Compute Reynolds number so the flow regime can be diagnosed.
    Re = reynolds(V, De, fluid.nu)

    # Choose which hydraulic equation will provide the friction losses.
    if method == 'darcy':
        ...
        rel_roughness = np.full_like(V, np.nan)
    # ------------------------------------------------------------------
    # Minor losses: expansion and contraction at each cross-section change
    # Based on Borda-Carnot (Streeter & Wylie 1985; Idel'chik 1986)
    # and cave conduit application in Jeannin (2001).
    #
    # At each internal node, compare V_nodes[i] (upstream) to V_nodes[i+1]
    # (downstream):
    #   expansion  (A increasing, V decreasing): h_e = K_e*(V1-V2)^2 / 2g
    #   contraction (A decreasing, V increasing): h_c = K_c * V2^2 / 2g
    # ------------------------------------------------------------------
    # Compare velocity changes between adjacent nodes to estimate minor losses
    # caused by expansions and contractions in passage size.
    ...
    # Expand segment diagnostics to node-length arrays for export convenience
...
# It is not solving real transient hydraulics; it just provides a sequence
# of steady discharges for repeated snapshot runs.
...
# Need containers here for HGL, EGL, and V for all times and all nodes.
# Output = time-series arrays that can later be animated or compared.
...
# --------------------------------------------------------------------
# Diagnostics and thesis-ready exports
# --------------------------------------------------------------------
...
# Output = a table of summary metrics for each scenario.
...
# Output = tables that show which parameters most affect head loss.
...
# Purpose: quantify how much the output could shift if geometry and discharge
# are slightly wrong.
...
# Need containers for many runs so percentile envelopes can be built later.
...
# --------------------------------------------------------------------
# 3D visualization geometry block.
# This is only for plotting.
# It wraps an ellipse around each centerline station using the LRUD dimensions
# so the cave can be displayed as a tube instead of just a line.
def generate_lrud_tube_from_tuples(centerline, shots, n_sides=50):
    n_nodes = centerline.shape[0]
    # Build arrays of left, right, up, and down measurements for every survey node.
    Ls, Rs, Us, Ds = [], [], [], []
    frm, to, L0, az, inc, L, U, D, R = shots[0]
    Ls.append(L)
    Rs.append(R)
    Us.append(U)
    Ds.append(D)
    ...
    for sh in shots:
        frm, to, Ls_, az, inc, L_, U_, D_, R_ = sh
        Ls.append(L_)
        Rs.append(R_)
        Us.append(U_)
        Ds.append(D_)

    Ls = np.asarray(Ls[:n_nodes], float)
    Rs = np.asarray(Rs[:n_nodes], float)
    Us = np.asarray(Us[:n_nodes], float)
    Ds = np.asarray(Ds[:n_nodes], float)

    widths = np.maximum(Ls + Rs, 1e-6)
    heights = np.maximum(Us + Ds, 1e-6)
    theta = np.linspace(0, 2 * np.pi, n_sides)

    X_tube = np.zeros((n_nodes, n_sides))
    Y_tube = np.zeros((n_nodes, n_sides))
    Z_tube = np.zeros((n_nodes, n_sides))

    for i in range(n_nodes):
        a = widths[i] / 2.0
        b = heights[i] / 2.0
        x0, y0, z0 = centerline[i]
        ex = a * np.cos(theta)
        ey = b * np.sin(theta)
        X_tube[i, :] = x0 + ex
        Y_tube[i, :] = y0 + ey
...
# Plot the 3D cave tube together with the hydraulic lines.
# This lets you visually compare the cave shape with the HGL and EGL results.
def plot_3d_tube_and_hgl(centerline, X_tube, Y_tube, Z_tube, results):
    centerline = np.asarray(centerline, float)
    x, y, z = centerline[:, 0], centerline[:, 1], centerline[:, 2]
    fig = go.Figure()

    if X_tube is not None and Y_tube is not None and Z_tube is not None:
        fig.add_trace(go.Surface(x=X_tube, y=Y_tube, z=Z_tube, opacity=0.35, showscale=False, name="LRUD tube"))

    fig.add_trace(go.Scatter3d(x=x, y=y, z=z, mode='lines', name='Centerline', line=dict(width=6)))

    for method, snap in results.items():
        HGL = np.asarray(snap['HGL'], float)
        EGL = np.asarray(snap['EGL'], float)
        fig.add_trace(go.Scatter3d(x=x, y=y, z=HGL, mode='lines', name=f'{method.upper()} HGL', line=dict(width=4, dash='solid')))
        fig.add_trace(go.Scatter3d(x=x, y=y, z=EGL, mode='lines', name=f'{method.upper()} EGL', line=dict(width=3, dash='dot')))

    fig.update_layout(
        title="3D Tube + HGL/EGL (snapshot)",
        scene=dict(aspectmode='data', xaxis_title='X', yaxis_title='Y', zaxis_title='Z / Head (m)'),
        legend=dict(itemsizing='constant')
    )
    return fig


def plot_heads_vs_distance(results, title="Head profiles vs distance"):
    fig = go.Figure()
    for method, snap in results.items():
        s = np.asarray(snap["s"], float)
        z = np.asarray(snap["z"], float)
        HGL = np.asarray(snap["HGL"], float)
        EGL = np.asarray(snap["EGL"], float)
        fig.add_trace(go.Scatter(x=s, y=HGL - z, mode="lines", name=f"{method.upper()} (HGL - z)"))
        fig.add_trace(go.Scatter(x=s, y=EGL - z, mode="lines", line=dict(dash="dash"), name=f"{method.upper()} (EGL - z)"))
    fig.update_layout(
        title=title,
        xaxis_title="Station distance s (m)",
        yaxis_title="Head above centerline z (m)",
        legend_title="Method / quantity",
        template="plotly_white"
    )
    return fig


def plot_uncertainty_envelope(df: pd.DataFrame, method: str, quantity: str = 'HGL'):
    if quantity == 'HGL':
        y05, y50, y95 = 'HGL_p05_m', 'HGL_p50_m', 'HGL_p95_m'
        ytitle = 'Hydraulic grade line (m)'
    else:
        y05, y50, y95 = 'V_p05_mps', 'V_p50_mps', 'V_p95_mps'
        ytitle = 'Velocity (m/s)'

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df['s_m'], y=df[y95], mode='lines', line=dict(width=0), showlegend=False))
    fig.add_trace(go.Scatter(
        x=df['s_m'], y=df[y05], mode='lines', fill='tonexty',
        name=f'{method.upper()} 5–95% envelope'
    ))
    fig.add_trace(go.Scatter(x=df['s_m'], y=df[y50], mode='lines', name=f'{method.upper()} median'))
    fig.update_layout(
        title=f"{method.upper()} uncertainty envelope ({quantity})",
        xaxis_title="Station distance s (m)",
        yaxis_title=ytitle,
        template="plotly_white",
    )
...
# Animation block for the flood pulse.
# It cycles through the steady snapshots so you can watch how modeled head
# or velocity changes through the synthetic flood event.
def animate_flood_pulse(centerline, X_tube, Y_tube, Z_tube, t_pulse, Q_pulse, snapshots,
                        methods=('darcy', 'manning', 'hazen'), color_by='pressure_head'):
    pts = np.asarray(centerline, float)
    x, y, z = pts[:, 0], pts[:, 1], pts[:, 2]
    n_steps = len(t_pulse)
    methods = list(methods)

    def get_colors(method, it):
        if color_by == 'velocity':
            return np.asarray(snapshots[method]['V'][it, :], float)
        return np.asarray(snapshots[method]['HGL'][it, :], float) - z

    allvals = []
    # Pre-allocate arrays to store HGL, EGL, and velocity for every time step.
    for m in methods:
        for it in range(n_steps):
            allvals.append(get_colors(m, it))
    allvals = np.hstack(allvals)
    ...
    for i, m in enumerate(methods):
        ...
        data.append(go.Scatter3d(
            x=x, y=y, z=z, mode='lines+markers', name=m, visible=(i == 0),
            line=dict(width=7),
            marker=dict(size=4, color=c0, cmin=vmin, cmax=vmax, colorbar=dict(title="HGL − z (m)"))
        ))
    ...
    for m in methods:
        ...
        frame_traces.append(go.Scatter3d(
            x=x, y=y, z=z, mode='lines+markers', line=dict(width=7),
            marker=dict(size=4, color=c, cmin=vmin, cmax=vmax), name=m
        ))
    frames.append(go.Frame(data=frame_traces, name=str(it)))

    dropdown = []
    for i, m in enumerate(methods):
        vis = [False] * len(methods)
        vis[i] = True
        dropdown.append(dict(label=m, method='update', args=[{'visible': vis}, {'title': f"3D Flood Pulse — {m}"}]))

    slider_steps = [
        dict(
            method='animate',
            args=[[str(it)], {'mode': 'immediate', 'frame': {'duration': 120, 'redraw': True}, 'transition': {'duration': 0}}],
            label=str(it)
        )
        for it in range(n_steps)
    ]
    ...
    fig = go.Figure(
        ...
        frames=frames,
        layout=go.Layout(
            title="3D Flood Pulse",
            scene=dict(aspectmode='data'),
            updatemenus=[
                dict(type='dropdown', x=0.02, y=0.98, buttons=dropdown),
                dict(type='buttons', x=0.02, y=0.86, buttons=[
...
class ValidationData:
    """
    Holds observed velocity measurements for model validation.

    Each entry represents one field measurement point:
      - s_obs   : along-passage distance (m) from the survey origin
      - V_obs   : measured mean flow velocity (m/s)
      - label   : optional site label (e.g. 'Scallop Site 1')

    How to populate:
      Load from a CSV with columns [s_m, V_obs_mps, label] using
      ValidationData.from_csv('validation_points.csv'), or build
      directly:
        ValidationData(
            s_obs   = np.array([10.2, 25.6, 44.1, 61.8]),
            V_obs   = np.array([0.127, 0.140, 0.160, 0.182]),
...
def run_discharge_closure_validation(
    ...
    """
    Validation strategy for velocity/discharge-only field data.

    Since V = Q/A is identical across methods, raw velocity comparison
    cannot distinguish them. Instead this function performs a discharge
    closure test:

      1. Interpolate each method's friction slope (Sf) to the observed
         scallop sites.
      2. Back-calculate velocity from Sf using each method's own
         governing equation.
      3. Back-calculate discharge as Q_back = V_back * A_site.
      4. Compare Q_back against the scallop-observed discharge
         Q_obs = V_obs * A_site.

    This tests whether each method's internal physics is consistent
    with the field-observed flow conditions, which is the meaningful
    comparison when HGL observations are unavailable.

    Metrics reported per method
    ---------------------------
    RMSE, MAE, PBIAS, NSE, R2  — all computed on discharge (m³/s)
    """
    point_rows   = []
    metrics_rows = []
...
def summarize_geometry_simple(geom: Geometry) -> pd.DataFrame:
    """Simple geometry summary using LRUD ellipses directly — no anchor scaling."""
    return pd.DataFrame({
        's_m':   geom.s,
        'x_m':   geom.x,
        'y_m':   geom.y,
...
# --------------------------------------------------------------------
# Main pipeline.
# This is the master function that ties the whole workflow together.
# Order of operations:
...
# This is the block to describe in your methods section as the overall workflow.
def run_full_model(
        paths: InputPaths,
    ...
    # Make sure the output folder exists before writing any files.
    outdir = ensure_output_dir(paths.output_dir)

    # 1) Read all inputs and build the basic survey geometry.
    shots = parse_survey_file(paths.survey_path)
    if len(shots) < 5:
        raise SystemExit("Not enough shots parsed — check parser/filename.")

    # Convert survey shots into 3D station coordinates.
    centerline = survey_to_xyz(shots)
    x, y, z = centerline[:, 0], centerline[:, 1], centerline[:, 2]
    n_nodes = len(x)

    # Convert LRUD values into elliptical hydraulic geometry at each node.
    ...
    # Map named anchor stations (for example MFS5) to Python node indices.
    ...
    # 2) Compute a baseline discharge from the scallop-derived velocities.
    # Read the scallop/velocity file and locate the velocity column.
    ...
    # Use the median site discharge as the representative baseline discharge.
    Q0 = float(np.median(Qi_sites))
    # 3) Calibrate a single Darcy friction factor from the scallop anchor sites.
    ...
    # Save the per-anchor Darcy calibration table.
    ...
    # 4) Run one steady-state snapshot for each hydraulic equation.
    results = {}
    for method in ['darcy', 'manning', 'hazen']:
        results[method] = simulate_snapshot(geom, Q0, coeffs, fluid, method=method)

    # 4b) Validate the model using observed/scallop-derived velocities.
    val_data = None
    validation_point_df   = pd.DataFrame()
    ...
    # REPLACE WITH:
    # Save validation outputs for site-by-site comparison and summary statistics.
    ...
    # 5) Export geometry tables and per-method node/anchor results.
    qa_df = pd.DataFrame({
    ...
    # 6) Sweep through alternative discharge and parameter values.
    ...
    # 7) Run Monte Carlo uncertainty analysis to build prediction envelopes.
    ...
    # 8) Generate a simple flood pulse as repeated steady snapshots.
    X_tube, Y_tube, Z_tube = generate_lrud_tube_from_tuples(centerline, shots, n_sides=50)
    t_pulse, Q_pulse, snapshots = compute_flood_pulse_snapshots(
        geom, fluid, coeffs, methods=('darcy', 'manning', 'hazen'),
        Q_base=Q0, Q_peak=Q0 * 5.0, n_steps=40
    )

    # 9) Save a metadata file that records the run settings and key outputs.
    metadata = {
        'Q0_m3s': Q0,
    ...
    if show_plots:
        plot_3d_tube_and_hgl(centerline, X_tube, Y_tube, Z_tube, results).show()
        plot_heads_vs_distance(results, title="Pressure head (HGL-z) and energy head (EGL-z) vs distance").show()
        for method, df in uncertainty.items():
            plot_uncertainty_envelope(df, method, quantity='HGL').show()
        animate_flood_pulse(centerline, X_tube, Y_tube, Z_tube, t_pulse, Q_pulse, snapshots).show()
        #plot_validation(
        #validation_point_df,
